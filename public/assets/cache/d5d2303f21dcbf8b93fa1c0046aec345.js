...
if(slideIndex=$this.attr('data-slide-to')){$target.data('bs.carousel').pause().to(slideIndex).cycle()}
e.preventDefault()})
...
if(actives&&actives.length){var hasData=actives.data('collapse')
...
this.$element[dimension](0)}
Collapse.prototype.reset=function(size){var dimension=this.dimension()
this.$element.removeClass('collapse')
[dimension](size||'auto')
[0].offsetWidth
this.$element[size!==null?'addClass':'removeClass']('collapse')
return this}
Collapse.prototype.transition=function(method,startEvent,completeEvent){var that=this
var complete=function(){if(startEvent.type=='show')that.reset()
...
that.enforceFocus()
transition?that.$element.one($.support.transition.end,function(){that.$element.focus().trigger('shown.bs.modal')}):that.$element.focus().trigger('shown.bs.modal')})}
...
Modal.prototype.hideWithTransition=function(){var that=this
var timeout=setTimeout(function(){that.$element.off($.support.transition.end)
that.hideModal()},500)
...
var $body=$(document.body).on('bs.modal.shown','.modal',function(){$body.addClass('modal-open')}).on('bs.modal.hidden','.modal',function(){$body.removeClass('modal-open')})}(window.jQuery);+function($){"use strict";var Tooltip=function(element,options){this.type=this.options=this.enabled=this.timeout=this.hoverState=this.$element=null
...
Popover.prototype.getContent=function(){var content=typeof this.options.content=='function'?this.options.content.call(this.$element[0]):this.options.content
...
ScrollSpy.prototype.refresh=function(){this.offsets=$([])
this.targets=$([])
var self=this
